...
app.post("/", (req, res) => {
  ...
  if (keyCount > 0) {
    res.json({
      message: `A much more exciting POST request - you posted me some data which had ${keyCount} key${keyCount > 1 ? 's' : ''}`,
    });
  } else {
    res.json({
      message: "This is a promising POST request - maybe attach some data, too?"
    })
  }
});
